...
const integrationSchema = new mongoose.Schema(
  {
    ...
    api: {
      ...
      authType: {
        type: String,
        enum: ['none', 'api-key', 'apikey', 'bearer', 'oauth2', 'basic'],
        default: 'api-key',
      },
    ...
    lastSync: {
      ...
      status: {
        type: String,
        enum: ['success', 'failed', 'partial', 'interrupted'],
      },
...
integrationSchema.methods.toSafeJSON = function () {
  ...
  if (obj.googleSheets) {
    obj.googleSheets.credentials = obj.googleSheets.credentials
      ? '[CREDENTIALS]'
      : undefined;
  }
